...
const Ql_user = () => {
  ...
  const fetchUsers = () => {
    fetch('http://localhost:3000/admin/user/role')
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      setAddUserState({
        ...
        role_id: 2, // Reset vai trò về mặc định
      }); // Reset form
    } catch (error) {
  ...
  return (
    <div className="admin_table_pra_wrapper">
        ...
        <h2 className="admin_table_pra_title">Quản lý User</h2>
          ...
          {users.map((user) => (
            <tr key={user.id_user}>
              ...
              <td className="admin_table_pra_role">
                {user.role_id === 0 ? 'Admin' : user.role_id === 1 ? 'Nhân viên' : 'User'}
              </td>
