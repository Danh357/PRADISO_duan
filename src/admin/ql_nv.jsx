...
const Ql_nv = () => {
  ...
  const fetchUsers = () => {
    fetch('http://localhost:3000/admin/nhanvien/role')
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Bạn có chắc muốn xóa người dùng này không?')) {
      fetch(`http://localhost:3000/admin/nhanvien/${id}`, { method: 'DELETE' })
        .then(() => {
  ...
  const handleSave = () => {
   ...
   if (sp.sdt_user && !phoneRegex.test(sp.sdt_user)) {
    ...
    const url = `http://localhost:3000/admin/nhanvien/${editUser || id}`;
    const opt = {
      method: 'PUT',
      headers: {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/admin/nhanvien', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
